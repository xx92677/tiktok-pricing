...
# 液态玻璃色板 (macOS 26 审美)
GLASS_BG = "#1C1C1E"       # 深空黑
GLASS_CARD = "#2C2C2E"     # 卡片色
GLASS_INPUT = "#3A3A3C"    # 输入框
GLASS_TEXT = "#F5F5F7"     # 主文字
GLASS_SUB = "#8E8E93"      # 辅助灰

COMMISSION_RATE = 0.0702   # 平台佣金 7.02%
FIXED_FEE_MYR = 0.54       # 固定服务费
TARGET_DISCOUNT = 0.51     # 预设折扣 51%
...
def calculate_logic(*args):
    ...
    try:
        ...
        # 确定马币汇率基准
        my_rate = FIXED_MYR_RATE if rate_mode.get() == "fixed" else base_rates["🇲🇾 MYR"]
...
root.geometry("620x920") # 宽度适配3列大字
root.configure(bg=GLASS_BG)
root.attributes("-alpha", 0.96) # 液态玻璃透明度
...
# --- 汇率区 (3列紧凑大字版) ---
lf = tk.LabelFrame(main, text=" 实时汇率设置 (锚定RMB) ", padx=10, pady=8, bg=GLASS_BG, fg=GLASS_SUB, font=("Arial", 10, "bold"))
