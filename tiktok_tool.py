...
# 汇率区 (极致压缩)
lf = tk.LabelFrame(main, text=" 实时汇率同步 ", padx=10, pady=5, bg=GLASS_BG, fg=GLASS_SUB, font=("Arial", 9, "bold"))
...
for i, curr in enumerate(FULL_RATES.keys()):
    r, c = (i // 3) + 1, (i % 3) * 2
    tk.Label(lf, text=f"{curr}:", font=("Arial", 8), bg=GLASS_BG, fg=GLASS_SUB).grid(row=r, column=c, sticky=tk.W)
    v = rmb_var if curr == "🇨🇳 人民币" else tk.StringVar(value=str(FULL_RATES[curr]))
    rate_vars[curr] = v
    e = tk.Entry(lf, width=8, textvariable=v, relief="flat", bg=GLASS_INPUT, fg=GLASS_TEXT, insertbackground="white")
...
# 输入与设置区
tk.Label(main, text="🎯 目标利润率 (Margin):", font=("Arial", 9, "bold"), bg=GLASS_BG, fg=GLASS_TEXT).pack(anchor=tk.W, pady=(2,2))
combo_profit = ttk.Combobox(main, values=[f"{i}%" for i in range(20, 31)], state="readonly")
combo_profit.set("20%")
combo_profit.pack(fill=tk.X, pady=(0, 10), ipady=4)
combo_profit.bind("<<ComboboxSelected>>", calculate_logic)

in_f = tk.Frame(main, bg=GLASS_BG); in_f.pack(fill=tk.X)
tk.Label(in_f, text="💰 成本 (RMB)", font=("Arial", 9), bg=GLASS_BG, fg=GLASS_TEXT).grid(row=0, column=0, sticky=tk.W)
entry_cost = tk.Entry(in_f, font=("Arial", 14), width=10, relief="flat", bg=GLASS_INPUT, fg=GLASS_TEXT, insertbackground="white")
...
tk.Label(in_f, text="🚚 运费及币种", font=("Arial", 9), bg=GLASS_BG, fg=GLASS_TEXT).grid(row=0, column=1, sticky=tk.W, padx=15)
s_b = tk.Frame(in_f, bg=GLASS_BG); s_b.grid(row=1, column=1, sticky=tk.W, padx=15)
entry_ship = tk.Entry(s_b, font=("Arial", 14), width=8, relief="flat", bg=GLASS_INPUT, fg=GLASS_TEXT, insertbackground="white")
entry_ship.pack(side=tk.LEFT); entry_ship.insert(0, "1.95")
entry_ship.bind("<KeyRelease>", lambda e: calculate_logic())

combo_ship = ttk.Combobox(s_b, values=list(FULL_RATES.keys()), width=8, state="readonly")
combo_ship.set("🇲🇾 马币"); combo_ship.pack(side=tk.LEFT, padx=5, ipady=3); combo_ship.bind("<<ComboboxSelected>>", calculate_logic)

tk.Label(main, text="💵 结果显示币种:", font=("Arial", 9), bg=GLASS_BG, fg=GLASS_TEXT).pack(anchor=tk.W, pady=(5,2))
combo_out = ttk.Combobox(main, values=list(FULL_RATES.keys()), state="readonly")
combo_out.set("🇲🇾 马币"); combo_out.pack(fill=tk.X, pady=(0, 10), ipady=4); combo_out.bind("<<ComboboxSelected>>", calculate_logic)

# 计算按钮 (紧凑设计)
btn = tk.Button(main, text="开 始 定 价", font=("Arial", 12, "bold"), 
                bg="#F5F5F7", fg="#1C1C1E", highlightthickness=0,
                relief="flat", command=calculate_logic, pady=8)
btn.pack(fill=tk.X, pady=(5, 15))
...
tk.Label(main, text="LIQUID GLASS UI • MAC SILICON NATIVE", font=("Arial", 8), fg="#48484A", bg=GLASS_BG).pack(pady=5)
