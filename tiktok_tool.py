...
FIXED_FEE_MYR = 0.54         
TARGET_DISCOUNT = 0.51       # 基础营销折扣 (折后价/0.49 = 原价)
...
def calculate_logic(*args):
    try:
        ...
        if bundle_mode.get():
            ...
            elif qty >= 3: bundle_discount = 0.7  # 3件及以上均为7折
        ...
        if divisor <= 0:
            ...
            text_res.insert(tk.END, "⚠️ 利润/广告/佣金/平台总和超标！")
        ...
        text_res.insert(tk.END, 
            f"【对账结果】 数量：{qty} 件 | 组合折：{int(bundle_discount*10)}折\n"
            ...
            f"达人预估 ({int(aff_pct*100)}%): {f_total_deal * aff_pct:.2f} {suffix}\n"
            f"平台费用 (7.02%): {f_total_deal * PLATFORM_COMMISSION:.2f} {suffix}\n"
            f"----------------------------------\n"
            f"订单成本总计: {to_out(total_base_expenses):.2f} {suffix}"
        )
        text_res.config(state=tk.DISABLED)
    except: pass
...
# --- 3. UI 构造 ---
root = tk.Tk()
root.title("TikTok 定价助手 Pro - 2026 旗舰版")
root.geometry("780x950")
...
# --- 汇率面板 (6列排布) ---
lf = tk.LabelFrame(main, text=" 汇率实时控制 ", padx=10, pady=8, bg=GLASS_BG, fg=GLASS_SUB, font=("Arial", 9, "bold"))
...
tk.Label(v_p, text="🎯 目标利润", font=("Arial", 9, "bold"), bg=GLASS_BG, fg=GLASS_TEXT).pack(anchor=tk.W)
combo_profit = ttk.Combobox(v_p, values=[f"{i}%" for i in range(16, 31)], state="readonly")
combo_profit.set("20%"); combo_profit.pack(fill=tk.X, pady=5, ipady=3); combo_profit.bind("<<ComboboxSelected>>", calculate_logic)

# 2. 广告支出
v_a = tk.Frame(op_frame, bg=GLASS_BG)
v_a.grid(row=0, column=1, sticky="ew", padx=10)
...
combo_ads.set("0%"); combo_ads.pack(fill=tk.X, pady=5, ipady=3); combo_ads.bind("<<ComboboxSelected>>", calculate_logic)

# 3. 达人佣金
v_aff = tk.Frame(op_frame, bg=GLASS_BG)
v_aff.grid(row=0, column=2, sticky="ew", padx=(0,10))
...
combo_aff.set("0%"); combo_aff.pack(fill=tk.X, pady=5, ipady=3); combo_aff.bind("<<ComboboxSelected>>", calculate_logic)

# 4. 多件折开关
v_sw = tk.Frame(op_frame, bg=GLASS_BG)
v_sw.grid(row=0, column=3, sticky="ew")
tk.Label(v_sw, text="📦 多件多折", font=("Arial", 9, "bold"), bg=GLASS_BG, fg=GLASS_TEXT).pack(anchor=tk.W)
bundle_mode = tk.BooleanVar(value=True)
tk.Checkbutton(v_sw, text="启用 (2件8折/3+件7折)", variable=bundle_mode, bg=GLASS_BG, fg=GLASS_SUB, selectcolor=GLASS_CARD, command=calculate_logic, font=("Arial", 8)).pack(pady=5, anchor=tk.W)
...
tk.Label(f1, text="💰 单件成本(RMB)", font=("Arial", 9), bg=GLASS_BG, fg=GLASS_TEXT).pack(anchor=tk.W)
entry_cost = tk.Entry(f1, font=("Arial", 14, "bold"), relief="flat", bg=GLASS_INPUT, fg=GLASS_TEXT, insertbackground="white")
entry_cost.pack(fill=tk.X, pady=5); entry_cost.insert(0, "20"); entry_cost.bind("<KeyRelease>", lambda e: calculate_logic())

# 2. 运费
f2 = tk.Frame(in_frame, bg=GLASS_BG)
f2.grid(row=0, column=1, sticky="ew", padx=10)
tk.Label(f2, text="🚚 总运费", font=("Arial", 9), bg=GLASS_BG, fg=GLASS_TEXT).pack(anchor=tk.W)
entry_ship = tk.Entry(f2, font=("Arial", 14, "bold"), relief="flat", bg=GLASS_INPUT, fg=GLASS_TEXT, insertbackground="white")
entry_ship.pack(fill=tk.X, pady=5); entry_ship.insert(0, "1.95"); entry_ship.bind("<KeyRelease>", lambda e: calculate_logic())

# 3. 币种
f3 = tk.Frame(in_frame, bg=GLASS_BG)
f3.grid(row=0, column=2, sticky="ew", padx=(0,10))
tk.Label(f3, text="运费币种", font=("Arial", 9), bg=GLASS_BG, fg=GLASS_TEXT).pack(anchor=tk.W)
combo_ship = ttk.Combobox(f3, values=list(FULL_RATES.keys()), state="readonly")
combo_ship.set("🇲🇾 MYR"); combo_ship.pack(fill=tk.X, pady=5, ipady=3); combo_ship.bind("<<ComboboxSelected>>", calculate_logic)

# 4. 数量
f4 = tk.Frame(in_frame, bg=GLASS_BG)
f4.grid(row=0, column=3, sticky="ew")
tk.Label(f4, text="🔢 数量", font=("Arial", 9), bg=GLASS_BG, fg=GLASS_TEXT).pack(anchor=tk.W)
combo_qty = ttk.Combobox(f4, values=[str(i) for i in range(1, 101)], state="readonly")
combo_qty.set("1"); combo_qty.pack(fill=tk.X, pady=5, ipady=3); combo_qty.bind("<<ComboboxSelected>>", calculate_logic)
...
btn = tk.Button(main, text="开 始 精 算 运 营 价 格", font=("Arial", 13, "bold"), bg="#F5F5F7", fg="#1C1C1E", highlightthickness=0, relief="flat", command=calculate_logic, pady=10)
