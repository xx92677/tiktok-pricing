...
def calculate_logic(*args):
    try:
        ...
        text_res.insert(tk.END, 
            ...
            f"成本总计: {to_out(cost_myr + ship_myr + FIXED_FEE_MYR):.2f}\n"
            f"--------------------\n"
...
# 汇率区
lf = tk.LabelFrame(main, text=" 实时汇率设置 ", padx=12, pady=10, bg=COLOR_BG, fg="#8E8E93")
...
for i, curr in enumerate(FULL_RATES.keys()):
    r, c = (i // 3) + 1, (i % 3) * 2
    tk.Label(lf, text=f"{curr}:", font=("Arial", 9), bg=COLOR_BG).grid(row=r, column=c, sticky=tk.W)
...
# 利润率选择 (20%-30%)
tk.Label(main, text="🎯 目标利润率 (Margin):", font=("Arial", 10, "bold"), bg=COLOR_BG).pack(anchor=tk.W, pady=(5,5))
...
# 输入
row_i = tk.Frame(main, bg=COLOR_BG); row_i.pack(fill=tk.X, pady=5)
tk.Label(row_i, text="💰 成本 (RMB):", font=("Arial", 10, "bold"), bg=COLOR_BG).grid(row=0, column=0, sticky=tk.W)
entry_cost = tk.Entry(row_i, font=("Arial", 15), width=12, relief="flat", highlightthickness=1, highlightbackground=COLOR_BORDER)
...
tk.Label(row_input_label := tk.Frame(row_i, bg=COLOR_BG), text="🚚 运费及币种:", font=("Arial", 10, "bold"), bg=COLOR_BG).grid(row=0, column=0)
row_input_label.grid(row=0, column=1, sticky=tk.W, padx=15)

s_b = tk.Frame(row_i, bg=COLOR_BG); s_b.grid(row=1, column=1, sticky=tk.W, padx=15)
entry_ship = tk.Entry(s_b, font=("Arial", 15), width=8, relief="flat", highlightthickness=1, highlightbackground=COLOR_BORDER)
...
tk.Label(main, text="💵 结果显示币种:", font=("Arial", 10, "bold"), bg=COLOR_BG).pack(anchor=tk.W, pady=(10,5))
...
btn = tk.Button(main, text="计 算 定 价", font=("Arial", 14, "bold"), 
                bg="#1D1D1F", fg="white", highlightbackground=COLOR_BG,
